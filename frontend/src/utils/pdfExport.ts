...
function generateRadarChartPage(pdf: jsPDF, audit: Audit, results: AuditResults, logoData: string | null = null): void {
  ...
  // Dimensions du graphique radar (paysage : page moins haute → radar plus compact)
...
function generateCorrectiveActionsPage(pdf: jsPDF, audit: Audit, logoData: string | null = null): void {
  ...
  for (let i = 0; i < totalRowsToShow; i++) {
    ...
    if (yPosition + rowHeight > pageHeight - 20) {
      // Ne pas ajouter de nouvelle page pour les lignes vides
      if (!rowData) break;
      pdf.addPage('a4', 'l');
      yPosition = 20;
    }
...
type AuditColWidths = ReturnType<typeof computeAuditColumnWidths>;
 ...
 * Pages en **paysage A4** pour gagner en largeur (commentaires / actions / photos).
 */
async function generateAuditDetailPages(pdf: jsPDF, audit: Audit, results: AuditResults, logoData: string | null = null): Promise<void> {
  console.log('Début génération pages audit détaillées');
  const margin = 10;
  ...
  const drawAuditPageHeader = (): number => {
    ...
    pdf.setFont('helvetica', 'normal');
    const adresseMaxWidth = pw - margin - 70 - 20;
  ...
  const drawTableColumnHeaders = (y: number, colWidths: AuditColWidths): number => {
  ...
  const drawCategoryBanner = (
    y: number,
    catIndex: number,
    categoryTitle: string,
    categoryScoreText: string,
    colWidths: AuditColWidths,
    ...
    pdf.setDrawColor(0, 0, 0);
    pdf.setLineWidth(0.5);
    pdf.rect(margin, y, availableW, categoryHeaderHeight, 'FD');

    pdf.setFontSize(10);
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(25, 118, 210);
    const categoryTitleText = `${catIndex + 1}. ${categoryTitle.toUpperCase()}${continuation ? ' (suite)' : ''}`;
    const titleY = y + categoryHeaderHeight / 2 + 2;
    ...
    if (categoryScoreText) {
      ...
      if (scoreX - (margin + 3 + titleWidth) < minSpace) {
        pdf.text(categoryScoreText, pw - margin - 5, titleY, { align: 'right' });
      } else {
        pdf.text(categoryScoreText, scoreX, titleY, { align: 'center' });
      }
    }

    return y + categoryHeaderHeight + 2;
  };

  // Parcourir les catégories - CHAQUE CATÉGORIE SUR SA PROPRE PAGE (paysage)
  for (let catIndex = 0; catIndex < audit.categories.length; catIndex++) {
    console.log(`Traitement catégorie ${catIndex + 1}/${audit.categories.length}`);
    ...
    // ---- NOUVELLE PAGE PAYSAGE POUR CHAQUE CATÉGORIE ----
    pdf.addPage('a4', 'l');
    ...
    yPosition = drawTableColumnHeaders(yPosition, colWidths);
    yPosition = drawCategoryBanner(yPosition, catIndex, categoryTitle, categoryScoreText, colWidths, false);

    // ---- Lignes des items ----
    for (let itemIdx = 0; itemIdx < auditedItems.length; itemIdx++) {
      const item = auditedItems[itemIdx];
      console.log(`  Item ${itemIdx + 1}/${auditedItems.length}: ${item.name}`);
      ...
      const bumpAuditPage = () => {
        ...
        yPosition = drawAuditPageHeader();
        yPosition = drawTableColumnHeaders(yPosition, colWidths);
        yPosition = drawCategoryBanner(yPosition, catIndex, categoryTitle, categoryScoreText, colWidths, true);
      };

      // Vérifier l'espace minimum nécessaire (au moins 35mm pour commencer un item)
      const bottomMargin = 20;
      const minSpaceToStartItem = 35;
      const currentSpace = pageHeight - yPosition - bottomMargin;
      if (currentSpace < minSpaceToStartItem) {
        console.log(`    Pas assez d'espace (${currentSpace}mm < ${minSpaceToStartItem}mm), nouvelle page`);
        bumpAuditPage();
      }
      ...
      // IMPORTANT: utiliser la même police/taille que le rendu pour éviter
      // la sous-estimation de hauteur (sinon texte coupé en bas de page).
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(7);
      ...
      // Photos : taille compacte, jusqu'à 4 par ligne côté à côté
      const photoGap = 2;
      const maxPhotoWidth = (colWidths.photos - photoGap * 5) / 4; // 4 photos par ligne avec marges
      const maxPhotoSize = Math.min(28, maxPhotoWidth); // Taille max 28mm (plus compact)
      ...
      const { commentFlat, actionFlat, alertFlat } = buildFlatObservationLines(
        commentLineArrays,
        actionLineArrays,
        commentTexts,
        rawNote
      ...
      const drawRowPart = async (
        ...
        pdf.setTextColor(0, 0, 0);
        if (isFirstPart) {
          const nameY = yPosition + 4;
          itemNameLines.forEach((line: string, idx: number) => {
            pdf.text(line, x + 3, nameY + (idx * lineHeight));
          });
        } else {
          // Indiquer que c'est une suite SEULEMENT si c'est vraiment une continuation
          pdf.setFontSize(7);
          pdf.setTextColor(100, 100, 100);
        ...
        for (let i = 0; i < commentSlice.length; i++) {
          if (alertSlice[i]) {
            pdf.setFillColor(255, 200, 200);
            pdf.rect(x + 1, commentY - 2, colWidths.comments - 2, lineHeight + 1, 'F');
            pdf.setTextColor(180, 0, 0);
          } else {
            pdf.setTextColor(0, 0, 0);
          }
          if (commentSlice[i] && commentSlice[i] !== '-' && commentSlice[i].trim()) {
            pdf.text(commentSlice[i], x + 2, commentY + 2.5);
          }
          commentY += lineHeight;
          pdf.setTextColor(0, 0, 0);
        }
        ...
        for (let i = 0; i < actionSlice.length; i++) {
          if (actionSlice[i] && actionSlice[i] !== '-' && actionSlice[i].trim()) {
            pdf.text(actionSlice[i], x + 2, actionY + 2.5);
          }
          actionY += lineHeight;
        }
        x += colWidths.actions;
        ...
        if (drawPhotos && numPhotos > 0 && item.photos) {
          ...
          let photosPlaced = 0;
          const maxPhotosToShow = Math.min(12, numPhotos);
          ...
          for (let i = 0; i < maxPhotosToShow; i++) {
            try {
              ...
              if (imgInfo) {
                ...
                // Nouvelle ligne après photosPerRow photos
                if (photosPlaced > 0 && photosPlaced % photosPerRow === 0) {
                  photoX = x + photoGap;
                  photoY += maxPhotoSize + photoGap;
                }
                
                // Vérifier qu'on ne dépasse pas la hauteur de la ligne
                if (photoY + finalHeight <= yPosition + rowHeight - photoGap) {
        ...
        } else if (!drawPhotos && numPhotos === 0) {
          pdf.setFontSize(7);
      ...
      const drawCompleteRow = async () => {
        ...
        await drawRowPart(
          commentFlat.length > 0 ? commentFlat : ['-'],
          actionFlat.length > 0 ? actionFlat : ['-'],
          alertFlat.length > 0 ? alertFlat : [false],
          totalRowHeight,
          true,
          true
        );
      ...
      if (totalRowHeight <= availableHeight) {
        // Cas 1 : tout tient sur la page actuelle
        await drawCompleteRow();
      } else {
        // Cas 2 : découpage nécessaire
        let remainingComments = [...commentFlat];
        let remainingActions = [...actionFlat];
        let remainingAlerts = [...alertFlat];
        let photosDrawn = false;
        let isFirstPart = true;
        let safetyCounter = 0;

        while ((remainingComments.length > 0 || remainingActions.length > 0 || !photosDrawn) && safetyCounter++ < 50) {
          const avail = pageHeight - yPosition - bottomMargin;

          if (isFirstPart) {
            const minH = Math.max(itemNameHeight, numPhotos > 0 ? photoContentHeight : 0, 20);
            if (avail < minH) { bumpAuditPage(); continue; }
            const extraLines = Math.max(0, Math.floor((avail - minH) / lineHeight));
            const take = Math.min(extraLines, Math.max(remainingComments.length, remainingActions.length));
            const cSlice = remainingComments.splice(0, take);
            const aSlice = remainingActions.splice(0, take);
            const alSlice = remainingAlerts.splice(0, take);
            const h = Math.max(minH, take > 0 ? take * lineHeight + 8 : 0);
            await drawRowPart(
              cSlice.length ? cSlice : (totalLines === 0 ? ['-'] : []),
              aSlice.length ? aSlice : (totalLines === 0 ? ['-'] : []),
              alSlice.length ? alSlice : [false],
              h, true, true
            );
            photosDrawn = true;
            isFirstPart = false;
          } else {
            if (!remainingComments.length && !remainingActions.length) break;
            if (avail < 15) { bumpAuditPage(); continue; }
            const maxLines = Math.max(1, Math.floor((avail - 8) / lineHeight));
            const take = Math.min(maxLines, Math.max(remainingComments.length, remainingActions.length));
            if (take < 1) { bumpAuditPage(); continue; }
            const cSlice = remainingComments.splice(0, take);
            const aSlice = remainingActions.splice(0, take);
            const alSlice = remainingAlerts.splice(0, take);
            // Seulement si on est sur une nouvelle page (pas sur la même)
            bumpAuditPage();
            await drawRowPart(
              cSlice.length ? cSlice : ['-'],
              aSlice.length ? aSlice : ['-'],
              alSlice.length ? alSlice : [false],
              take * lineHeight + 8, false, false
            );
          }
        }
      }
  ...
  let footerPageW = pdf.internal.pageSize.getWidth();
  let footerPageH = pdf.internal.pageSize.getHeight();

  // Calculer la position Y sur cette page - placer le footer en bas
  let footerY = footerPageH - 30;

  // Si la dernière page est trop remplie, ajouter une page paysage pour le footer
  if (footerY < 60) {
    pdf.addPage('a4', 'l');
    footerPageW = pdf.internal.pageSize.getWidth();
    footerPageH = pdf.internal.pageSize.getHeight();
    footerY = footerPageH - 30;
  }
  ...
  const footerTableWidth = footerPageW - 2 * margin;
  const footerRowHeight = 22; // Légende sur plusieurs lignes + note impression couleur
  ...
  pdf.setTextColor(0, 0, 0);
  const legendText =
    '* 1 : conforme ; 0,7 : non-conformité mineure ; 0,3 : non-conformité moyenne ; 0 : non-conformité majeure. ' +
    'Pour une impression fidèle aux couleurs (pastilles, tableaux), utiliser une imprimante couleur et éviter le mode « noir et blanc » ou « économie d’encre ».';
