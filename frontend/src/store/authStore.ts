...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  login: async (email, password) => {
    try {
      ...
      if (userData) {
        ...
        // Réinitialiser l'audit en cours : évite qu'un auditeur voie l'audit
        // d'un collègue encore en mémoire depuis la session précédente.
        import('./auditStore').then(({ useAuditStore }) => {
          const { currentAudit } = useAuditStore.getState();
          if (currentAudit && currentAudit.auditorId !== user.id) {
            useAuditStore.setState({ currentAudit: null, audits: [], results: null });
          }
        }).catch(() => {});
  ...
  logout: () => {
    authApi.logout();
    // Réinitialiser l'audit en cours pour que le prochain utilisateur
    // connecté sur ce navigateur ne voie pas l'audit de la session précédente.
    import('./auditStore').then(({ useAuditStore }) => {
      useAuditStore.setState({ currentAudit: null, audits: [], results: null });
    }).catch(() => {});
    set({ 
